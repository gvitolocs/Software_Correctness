...
public class DrawingCanvas extends Pane {
    ...
    private static final double HOVER_OFFSET = 12;
    private double hoverCoordinateDivisor = 1.0;
    private double drawingCoordinateDivisor = 1.0;
    ...
    public DrawingCanvas() {
        ...
        canvas.setOnMouseDragged(e -> updateHover(e.getX(), e.getY()));
        canvas.setOnMouseExited(e -> hoverLabel.setVisible(false));
    }

    private void updateHover(double px, double py) {
        double x = fromPixelX(px) / hoverCoordinateDivisor;
        double y = fromPixelY(py) / hoverCoordinateDivisor;
        hoverLabel.setText(String.format("x=%.1f  y=%.1f", x, y));
        double w = getCanvasWidth();
        double h = getCanvasHeight();
        double lw = hoverLabel.prefWidth(-1);
        double lh = hoverLabel.prefHeight(-1);
        double lx = px + HOVER_OFFSET;
        double ly = py + HOVER_OFFSET;
        if (lx + lw > w) lx = px - HOVER_OFFSET - lw;
        if (ly + lh > h) ly = py - HOVER_OFFSET - lh;
        hoverLabel.setLayoutX(Math.max(0, lx));
        hoverLabel.setLayoutY(Math.max(0, ly));
        hoverLabel.setVisible(true);
    }

    public void setHoverCoordinateDivisor(double divisor) {
        hoverCoordinateDivisor = divisor > 0 ? divisor : 1.0;
    }

    public void setDrawingCoordinateDivisor(double divisor) {
        drawingCoordinateDivisor = divisor > 0 ? divisor : 1.0;
    }

    private double fromPixelX(double px) {
        double scale = getScale();
        if (scale == 0) return boxX1;
        return boxX1 + ((px - MARGIN) / scale) * drawingCoordinateDivisor;
    }

    private double fromPixelY(double py) {
        double scale = getScale();
        if (scale == 0) return boxY1;
        return boxY1 + ((getCanvasHeight() - MARGIN - py) / scale) * drawingCoordinateDivisor;
    }
    ...
    private double toPixelX(double x) {
        double scale = getScale();
        return MARGIN + ((x - boxX1) / drawingCoordinateDivisor) * scale;
    }

    private double toPixelY(double y) {
        double scale = getScale();
        return getCanvasHeight() - MARGIN - ((y - boxY1) / drawingCoordinateDivisor) * scale;
    }
    ...
    private double getScale() {
        ...
        double visualBoxW = boxW / drawingCoordinateDivisor;
        double visualBoxH = boxH / drawingCoordinateDivisor;
        if (visualBoxW <= 0 || visualBoxH <= 0) return 1;
        double scaleX = (w - 2 * MARGIN) / visualBoxW;
        double scaleY = (h - 2 * MARGIN) / visualBoxH;
    ...
    private void drawShapeLine(String line) {
        ...
        try {
            switch (kind) {
                ...
                case "TEXT" -> {
                    if (parts.length >= 5) {
                        double x = parseDecimalCoordinate(parts[1]);
                        double y = parseDecimalCoordinate(parts[2]);
    ...
    private double parseDecimalCoordinate(String raw) {
